...
use crate::components::dashboard::{
    fetch_satellite_detail, fetch_satellite_logs, fetch_satellites, format_time, LogEntry,
    SatelliteDetail,
};
...
fn build_view(name: &str, specs: Option<&SatelliteDetail>, entries: &[LogEntry]) -> Option<SatView> {
...
pub fn Orbit() -> impl IntoView {
    ...
    spawn_local(async move {
        ...
        let mut specs_cache: HashMap<String, SatelliteDetail> = HashMap::new();
                ...
                Ok(names) => {
                    ...
                    for name in &names {
                        ...
                        if !specs_cache.contains_key(name) {
                            if let Ok(spec) = fetch_satellite_detail(name).await {
